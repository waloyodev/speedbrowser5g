    ...
    private boolean isSpecializedHandlerAvailable(@NonNull Intent intent) {
        ...
        for (ResolveInfo resolveInfo : handlers) {
            ...
            if (filter == null) {
                ...
                // Error on the side of staying in the waloyodev, ignore
                continue;
            }
            // NOTICE: Use of && instead of || will cause the waloyodev
            // to launch a new intent for every URL, using OR only
            // launches a new one if there is a non-waloyodev app that
            // can handle it.
